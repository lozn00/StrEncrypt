...
public class JWUtils {
	...
	/**
	 * 解析命令行参数信息 -s 1 -m JW -r int -p [int,java.lang.String] -c JWUtils
	 * 
	 * @param cmdLine
	 */
	public static void parseCmdLineArgs(String cmdLine) {
		try {
			...
			for (int i = 0; i < strAry.length; i += 2) {
				...
				case "-p":
					...
					try {
						String[] arys = filterMethodParamType.substring(1, filterMethodParamType.length() - 1)
								.split(",");
						StringBuilder sb = new StringBuilder();
